export default function AvatarCoreMini({ listening, speaking, thinking }) {
  const state = thinking ? "thinking" : speaking ? "speaking" : listening ? "listening" : "idle";
  return (
    <div className="jarvis-core jarvis-core-mini" data-state={state}>
      <svg viewBox="0 0 400 400" className="jarvis-core-svg">
        <defs>
          <radialGradient id="coreGlowMini" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#8FE9FF" stopOpacity="0.9" />
            <stop offset="45%" stopColor="#38BDF8" stopOpacity="0.35" />
            <stop offset="100%" stopColor="#38BDF8" stopOpacity="0" />
          </radialGradient>
        </defs>
        <circle cx="200" cy="200" r="150" fill="url(#coreGlowMini)" className="core-halo" />
        {[150, 118, 90].map((r, i) => (
          <circle key={r} cx="200" cy="200" r={r} fill="none" stroke="#38BDF8"
            strokeOpacity={0.35 - i * 0.08} strokeWidth="1" className={`core-ring core-ring-${i}`} />
        ))}
